    ...
    for reg in regL:
        for lAmbda in lambdaL:
            for alpha in alphaL:
                ...
                acc = calAcc(predH2O["predict"].as_data_frame(use_pandas=False), Dtest["C11"].as_data_frame(use_pandas=False))
                if (acc > bestAccLH):
                    bestAccLH = acc
                ...
                model2.train(x=list(range(0,10)), y="C11", training_frame=Dtrain)
                predH2O2 = model2.predict(Dtest)
                acc = calAcc(Dtest["C11"].as_data_frame(use_pandas=False), predH2O2['predict'].as_data_frame(use_pandas=False))
                if (bestAccSQERR < acc):
                    bestAccSQERR = acc;
